"""FHWA ADAS two-vehicle CSV -> Ozone (NBDT) trajectory + metadata.

Partitioning: one output folder per field collection, keyed by
``date``, ``time of day``, ``route starting point(rs)``, ``route ending point(re)``.
Folder names follow ``{corridor}_{YYYYMMDD}_{HHMMSS}_run{run}_sub{sub}``.

Each raw row expands to three trajectory records (SV1, SV2, AdjV).
"""
...
VEHICLE_ROLE_SV = 2
VEHICLE_ROLE_ADJ = 1
DATASET_NAME = "ADAS_TwoVehicle_Ohio"
...
def build_agent_records(
    ...
    if np.isscalar(car_id) or isinstance(car_id, str):
        car_id_values = np.full(n, car_id, dtype=object)
    else:
        car_id_values = np.asarray(car_id, dtype=object)
...
def _trajectory_sort_key(df: pd.DataFrame) -> pd.DataFrame:
    role_order = {SV1_CAR_ID: 0, SV2_CAR_ID: 1}

    def sort_rank(car_id) -> tuple:
        if car_id in role_order:
            return (0, role_order[car_id])
        try:
            return (1, int(car_id))
    ...
    out = df.copy()
    out["_sort_rank"] = out["carId"].map(sort_rank)
    out = out.sort_values(["frameNum", "_sort_rank"]).drop(columns="_sort_rank")
    return out.reset_index(drop=True)


class ADASTwoVehicleTransfer(BasicTransfer):
    """Convert FHWA ADAS two-vehicle naturalistic CSV to Ozone format."""

    DEFAULT_CSV = (
        "Advanced_Driver_Assistance_System__ADAS_-Equipped_"
        "Two-Vehicle_Data_for_Central_Ohio.csv"
    )
    ...
    def build_metadata_row(segment: pd.DataFrame) -> dict:
        ...
        extras = {
            "corridor": ADASTwoVehicleTransfer.corridor_from_routes(
            ...
            "speed_limits": ADASTwoVehicleTransfer.convert_speed_limits_to_mps(
                first_non_null(segment["speed limits"])
            ),
            "route_distance": ADASTwoVehicleTransfer.parse_route_distance_to_m(
            ...
            "gap_level": first_non_null(segment["gap level"]),
    ...
    def get_csv_path(self) -> str:
        ...
        if os.path.isfile(default):
            return default
        rows_csv = os.path.join(folder, "two_vehicle_data", "rows.csv")
        if os.path.isfile(rows_csv):
            return rows_csv
        for name in os.listdir(folder):
            if name.endswith(".csv") and "Two-Vehicle" in name:
                return os.path.join(folder, name)
        raise FileNotFoundError(f"No ADAS two-vehicle CSV found under {folder}")
    ...
    def _process_segment(self, segment: pd.DataFrame) -> pd.DataFrame:
        ...
        on many rows. Emitting SV1/SV2 records per raw row duplicates
        ``(frameNum, carId)`` for ``0_1`` / ``0_2`` and can assign conflicting
        subject-vehicle states at the same frame.
        """
        ...
        sv1 = build_agent_records(
            frame_num=time_to_frame_num(sv_source["Time"].values),
            car_id=SV1_CAR_ID,
            east_m=sv_source["pos_x_sv1_m"].values,
            north_m=sv_source["pos_y_sv1_m"].values,
            heading_map=sv_source["heading_sv1"].values,
            speed=sv_source["speed_sv1"].values,
            length_m=sv_source["dim_x_sv1"].values,
            width_m=sv_source["dim_y_sv1"].values,
            height_m=sv_source["dim_z_sv1"].values,
            lane_id=sv_source["lane_id_sv1"].values,
            vehicle_role=VEHICLE_ROLE_SV,
        ...
        sv2 = build_agent_records(
            frame_num=time_to_frame_num(sv_source["Time"].values),
            car_id=SV2_CAR_ID,
            east_m=sv_source["pos_x_sv2_m"].values,
            north_m=sv_source["pos_y_sv2_m"].values,
            heading_map=sv_source["heading_sv2"].values,
            speed=sv_source["speed_sv2"].values,
            length_m=sv_source["dim_x_sv2"].values,
            width_m=sv_source["dim_y_sv2"].values,
            height_m=sv_source["dim_z_sv2"].values,
            lane_id=sv_source["lane_id_sv2"].values,
            vehicle_role=VEHICLE_ROLE_SV,
            type_of_vehicle=type_of_vehicle.loc[sv_source.index],
            origin_lon=sv_source["map_origin_x"].values.astype(np.float64),
            origin_lat=sv_source["map_origin_y"].values.astype(np.float64),
        ...
        combined = pd.concat([sv1, sv2, av], ignore_index=True)
        combined = _trajectory_sort_key(combined).drop_duplicates(
    ...
    def _process_data(self, file_path: str) -> pd.DataFrame:
        raise NotImplementedError(
            "ADASTwoVehicleTransfer writes numbered folders; use run() instead."
        )
    ...
    def run(self) -> None:
        ...
        print(f"  ADAS two-vehicle: {csv_path} → {len(collection_keys)} collections")
